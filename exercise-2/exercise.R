...
for(i in 1:100) {
  ...
  training.testing <- sample(seq_len(nrow(homes)), size = sample.size)
  training.data <- homes[training.testing,]
  test.data <- homes[-training.testing,]
  
  # 2. Pass your **training data** to the `rpart` function to run a simple classification operation
  class <- rpart(in_sf ~ ., data = training.data, method="class")
  
  # 3. Pass your results to the `AssessFit` function to assess the fit
  fits <- AssessFit(class)
  
  # 4. Store your assessment in the `basic.fits` vector
  basic.fits <- c(basic.fits, fits)
}
